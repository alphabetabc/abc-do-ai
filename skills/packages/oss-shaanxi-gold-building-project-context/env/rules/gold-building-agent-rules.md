---
description: 金楼项目 AI 协作工作守则：路径引用只读、模块索引、Do-Not 禁止清单（含 docs 会签与单向引用）。
alwaysApply: true
---

# 金楼项目 · AI 协作工作守则（项目级 · alwaysApply）

> **来源**：本文件由 skill 的 `env/AGENTS.md` 转写为 AI 助手级 rule，原 `.trae/AGENTS.md` symlink 已废弃删除。
> **策略**：`alwaysApply: true` —— 本项目内始终生效。
...
仓库顶层结构（各目录与所属详细结构见 `oss-shaanxi-gold-building-project-context` 的 `design/codebase-map.md`，按需读）：
