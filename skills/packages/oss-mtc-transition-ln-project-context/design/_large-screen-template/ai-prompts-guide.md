...
6. **主动识别共享能力**：生成 spec 时，主动识别可被多个大屏复用的公共组件/能力，
   ...
   生成共享 spec 前必须检查 docs/specs/ 下是否已有共享组件 spec 目录。已存在则直接引用，不重复创建。
   **人确认环节**：AI 标记"建议共享"后，需人 review 确认"确认共享"或"不共享"。
   只有"确认共享"的才进入 tasks 前置，"不共享"的留在本 spec 内。
   **强约束**：标记"确认共享"的，tasks.md 必须在「前置：共享能力」段列出对应 task，
   且必须先于业务任务完成，不能跳过。
...
大屏开发过程中，识别出可被多个大屏复用的公共组件/能力时，直接生成共享组件 spec。

### 3.1 何时生成

当某个组件/能力**不只是当前大屏使用**，且尚未作为共享能力存在时，应在共享组件 spec 目录下新增组件子目录。

### 3.2 生成前检查

生成前**必须检查**，已存在则直接引用，不重复创建：
...
> 共享组件 spec 使用**三件套**（不是五件套），因为组件不是独立交付特性。

### 3.4 生成共享组件 spec（复制即用）

将 `{NNN}` 替换为共享 spec 编号（如 `042`），`{组件名}` 替换为组件英文名（如 `ec-map`）。

```text
请基于 @docs/specs/{编号}-{slug}/spec.md §8「共享能力」中登记的组件，
在 docs/specs/{NNN}-components-common/{NNN}-{组件名}/ 下生成共享组件三件套。

【生成前检查】
1. 检查 docs/specs/ 下是否已有共享组件 spec 目录（NNN-components-common）
2. 检查该目录下是否已有同类组件子目录
若已存在则直接引用，不重复创建。

【输出】
在 docs/specs/{NNN}-components-common/{NNN}-{组件名}/ 下生成三件套：
- spec.md：组件契约（props、行为、三态）
- task.md：开发任务
- data-model.md：组件数据结构（如有）

【约束】
1. 组件契约从源 spec 的需求推导，不要另起一套设计。
2. 三件套精简，不需要 plan 和 acceptance-tests。
3. 生成后在源 spec §8「共享能力」表中回填共享 spec 引用路径。
4. 生成后在源 spec tasks.md「前置：共享能力」段列出对应 task 引用。
```
