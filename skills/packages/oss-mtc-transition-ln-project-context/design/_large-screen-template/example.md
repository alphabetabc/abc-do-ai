...
> 以 038 人员信息大屏为例，展示大屏五件套 + 共享组件 spec 的完整目录结构。
...
docs/
├── specs/
│   ├── 038-bigdata-personnel-display/          ← 大屏五件套
│   │   ├── pm-inputs/
│   │   │   ├── assets/                         ← PM 原始素材
│   │   │   │   ├── 大数据综合展示-人员信息.txt
...
│   │
│   ├── 042-components-common/                  ← 共享组件 spec（独立编号）
...
│       ├── spec.md                             ← §8 共享能力：引用已有的 042-components-common
│       ├── plan.md
...
① 038 五件套生成
   ...
   spec §8 AI 识别出 3 个共享能力（ec-map、shell、pie-chart），标记"建议共享"
   人 review 确认 → 标记"确认共享"

② 生成共享组件 spec
   手动调用生成 prompt → 在 042-components-common/ 下生成三件套
   回填 038 spec §8 引用路径 + tasks 前置引用

③ 039 五件套生成
   spec §8 检查 042-components-common/ → 已存在，直接引用
   tasks 前置段引用已有共享能力，不重复创建
```
