...
**镜像位置**：仓库根还有一份 `AGENTS.md`，是本目录下 `AGENTS.md` 的**硬链接镜像**（对外暴露给团队）。任何对本目录的修改都会自动反映到根目录，无需手动同步。

**语义分工**：

- `env/AGENTS.md` = 源（canonical，私人、skill 内）
- 根 `AGENTS.md` = 镜像（对外暴露）

> ⚠️ NTFS 硬链接在物理上是对等的（共享 inode，无方向）；这里讲的"源 / 镜像"是**分工**上的语义，不是文件系统的强制关系。

若需重新建立硬链接（在跨机器克隆或文件丢失时），执行：
...
若要替换为拷贝版本（不再硬链接），执行 `Copy-Item .\.trae\skills\oss-mtc-transition-ln-project-context\env\AGENTS.md .\AGENTS.md -Force` 即可。

**管理范围**（详见 AGENTS.md §2.1）：

| 模块 | 前端路由 | Spec |
|------|---------|------|
| 人员信息大屏 | `/visual/big-screen/personnel`（计划） | 待新建 |
| 信访信息大屏 | `/visual/big-screen/petition`（计划） | 待新建 |
| 大数据综合展示 | `/visual/big-screen` | `035-visual-monthly-statistics/spec.md` §1.4 提到的「另 spec」 |
| 统计分析月报 | `/visual/stats/*` | `035-visual-monthly-statistics` |

**约定要点**（详见 AGENTS.md §3–§7）：

- 前端：大屏代码集中放 `frontend/src/pages/visual/{big-screen,stats}/` 与 `components/big-screen/`
...
```json
{
  ...
  > ⚠️ 此处内容由 `package.json`（**硬链接镜像**）提供，外部修改会自动反映。若需重建硬链接，执行：
>
> ```powershell
> New-Item -ItemType HardLink `
