...
| 038  | bigdata-personnel-display（**人员信息大屏**）               | 五件套已生成；PM 输入已迁入；编码待启动（task-016） |
| 039  | bigdata-petition-display（**辽宁信访大屏**）                | 仅有 PM 输入，未生成五件套                          |
| 040  | bigdata-beijing-petition-display（**进京信访大屏**）        | 仅有 PM 输入，未生成五件套                          |
| 041  | bigdata-petition-comparison-display（**信访数据比对大屏**） | 仅有 PM 输入，未生成五件套                          |

> 前端路由详见 `env/project-meta.md` §1（skill 内部路由信息唯一权威记录点）。路由以 PM 输入原文为权威（见 memo R7）。
...
| **可视化分析 / 大屏（035 + 038–041）** | 035 月报 M1–M3 已实现，M4 待建；038 人员大屏五件套已生成+M0 文档审批完成，编码待启动；039–041 仅有 PM 输入 |
