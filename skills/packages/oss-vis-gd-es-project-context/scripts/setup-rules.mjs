...
const DEFAULT_TARGET_DIRS = [".trae/rules", ".cursor/rules", ".claude/rules"];
...
Options:
  --target, -t    项目根绝对路径（必填）
  --dirs,  -d     目标 IDE rules 目录，逗号分隔（默认 .trae/rules,.cursor/rules,.claude/rules）
  ...
  for (const targetRelDir of args.dirs) {
    ...
    for (const fname of sources) {
      ...
      try {
        ...
        if (r.ok && samePath(r.absTarget, resolve(sourceAbs))) {
          console.log(`  [+] ${fname} created (fs reported "${e.code}" but link is healthy)`);
          totalCreated++;
          continue;
        }
