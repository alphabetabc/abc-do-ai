...
| `mapWeatherWarningLevelTypes` | `Array<{ name: string, value: string, type: string }>` | 预警级别硬编码映射表，固定五级：白色(1)、蓝色(2)、黄色(3)、橙色(4)、红色(5)。`type` 用于图标工厂的动态选择。 |
...
**图标工厂说明**：每个图例项包含一个 `Icon` 组件，根据传递的 `type`（来自预警级别）选择对应颜色的图标路径。图标大小为 48x48px。
...
| `zoneId` | `string` | 用于点位聚合匹配 |
| `zoneName` | `string` | 用于点位名称匹配和经纬度补全 |
| `longitude` / `latitude` | `string` | 经纬度，缺失时会从 `labelPoints` 中按名称匹配 |
