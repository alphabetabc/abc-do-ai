...
| `bagNum`       | `string \| number` | 用于 `suggestion` 格式化      |
| `oilEngineNum` | `string \| number` | 用于 `suggestion` 格式化      |

### 2. InfoCard.tsx（省级统计卡片 + 退服趋势图）

#### 交互

1. **全局显示**：省级统计卡片固定定位在地图右上角（right: 60px, top: 30px），800px 宽度，受高风险乡镇图例控制显示/隐藏。
...
**API 1：getHighRiskTownInfoCardApi**

| 项目         | 说明                                                                               |
| ------------ | ---------------------------------------------------------------------------------- |
| 触发时机     | 组件挂载                                                                           |
| 请求参数     | 无（固定查广东省数据）                                                             |
| 响应数据处理 | 调用 `getInfo(rows)` 提取摘要、`getTableDataSource(rows)` 转换表格，默认选中第一行 |
| 刷新机制     | 仅在组件挂载时请求一次，无轮询                                                     |
...
| `viewPageId`   | `guarantee-middle-page`                  |
| `viewItemId`   | `high-risk-city-detail`                  |
| `viewPageArgs` | `{ zoneName: "广东省", zoneLevel: "2" }` |

**响应数据**：与 `getHighRiskTownMapDetailApi` 相同的 `rows` 结构，额外包含 `forecastDataTime` 字段。
