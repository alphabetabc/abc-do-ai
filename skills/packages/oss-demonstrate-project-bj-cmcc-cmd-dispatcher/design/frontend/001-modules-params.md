...
-   **结构型叠加变种**（`station-outage`）：保留 UI 出图作为背景（标题栏 + 装饰），上层叠加交互组件（tab / 表格 / 折线图 / 日期选择）。背景图 `pointerEvents: none`，交互层正常响应
...
> 版本：v1.1.0
> 日期：2026-08-25（v1.1.0：station-outage 形态变更 — 图片型 → 结构型叠加 + 新增"结构型叠加变种"约定）
