...
| `modules/map/map-detail-modal.tsx`               | 框选区域详情弹窗（task002-03 落地 + task011 PM 微调）：仅 `street` 层级 + `modalOpen=true` + `selectedId` 存在时弹出 `<img>` 弹窗；task011 PM 微调：弹窗位置 `right: 50%→44% / top: 40%→30%`（更靠左上 6% / 上移 10%）、`zIndex: 10→100`（叠在 Background 之上）、弹窗图片 `width: 420→620`（演示放大便于看清）；业务逻辑（仅 street 守卫 + 关闭按钮 `onClose` + `constants.IMAGE_PATH` 引用 + 注释掉的智能避让代码块）保持不变 |
...
| `modules/service-recovery/`                       | MY_LEVELS: city/company/district/street；退服恢复情况，复用 `service-recovery-panel`，variant='city'，引用 `退服恢复情况.png`；task007 新增时间范围 label（mount 时快照：此刻往前 2 小时 → 此刻，格式 `yyyy-mm-dd hh:mm:ss 至 yyyy-mm-dd hh:mm:ss`，浮在图片左上角 left:93/top:832，zIndex:100）；task011 新增模块最上方自定义时间段 RangePicker（`DatePicker.RangePicker`，默认值 `[now-2h, now]`，`format="YYYY-MM-DD HH:mm:ss"` + `showTime`，`size="large"`，位置 `left:1064 / top:823 / zIndex:100`，纯本地 `customRange` state，**不联动** TimelineHistory / store / 趋势图）                                                                                                                                                                      |
...
| `map/company/outline.png`                        | company 层底图轮廓（task011 替换新版：83418 → 95694 bytes）            | 同上                                                                                     |
...
| `地图弹窗-1.png`                                  | 框选区域详情弹窗内容图（task011 替换新版高分辨率：167989 → 479376 bytes） | `map-detail-modal.tsx` 通过 `constants.IMAGE_PATH` 引用                                  |
...
-   [x] [task-2026-08-27-011-service-recovery-range-picker.md](../plans/done/task-2026-08-27-011-service-recovery-range-picker.md)：覆盖两个目标 ——
    - **目标 A**：退服恢复模块顶部新增自定义时间段 RangePicker（`DatePicker.RangePicker`，默认 `[now-2h, now]`，纯本地 state 不联动下游；保留 task007 原快照 label；位置 `left:1064 / top:823 / size="large"`）
    - **目标 B**：map 弹窗样式微调 + 图片资源更新（`modules/map/map-detail-modal.tsx` 弹窗位置 `right: 50%→44% / top: 40%→30%` / `zIndex: 10→100` / 图片 `width: 420→620`；`map/company/outline.png` 替换 83418 → 95694 bytes；`地图弹窗-1.png` 替换 167989 → 479376 bytes）

---

## 文档元信息

> 日期：2026-08-27（更新：task011 两个目标同步归档——
> **目标 A**：退服恢复模块顶部新增自定义时间段 RangePicker——`modules/service-recovery/index.tsx` 新增 `customRange` 本地 state + `<DatePicker.RangePicker>`，默认 `[now-2h, now]`，位置 `left:1064 / top:823 / size="large"`，纯本地 state 不联动 TimelineHistory / store / 趋势图；保留 task007 原 `timeRange` 快照 label 不动
> **目标 B**：map 弹窗样式微调 + 图片资源更新——`modules/map/map-detail-modal.tsx` 弹窗位置 `right: 50%→44% / top: 40%→30%` / `zIndex: 10→100` / 弹窗图片 `width: 420→620`；`map/company/outline.png` 替换新版（83418 → 95694 bytes）；`地图弹窗-1.png` 替换新版高分辨率版本（167989 → 479376 bytes）；业务逻辑（仅 street 层级守卫 + modalOpen + selectedId + 关闭按钮 onClose + 注释掉的智能避让代码块）保持不变）
> 历史：
>
> -   2026-08-26：task010 趋势图 label 数据/视图分离 + PM 截断规则——9 个 mock 移除 label 字段；trend-chart.tsx 与 station-outage/index.tsx 新增 anchorMs + xLabels useMemo；markLine 改用索引定位；service-recovery / station-outage 模块描述更新；mock 清单追加 station-outage 5 行
