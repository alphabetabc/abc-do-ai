...
| `modules/station-outage/` | MY_LEVELS: city~community；基站退服，引用 `基站退服.png` |
...
| 文件 | 用途 | 引用方式 |
|---|---|---|
| `map-markers.json` | 地图打点数据（7 个层级；按 task008 计划最终 117 个点位，前五层 113 节点+资源 + station 3 + logical 1）；当前 L1 city 已完成 36 个点位，其余 L2~L5 见 [task-2026-08-25-008-map-markers-mock.md](../plans/task-2026-08-25-008-map-markers-mock.md) 进行中 | `map-stage.tsx` 通过 `useRequest` + `constants.STATIC_PATH` fetch |
...
| Level | 中文 | 可见模块 |
|---|---|---|
| `city`（默认） | 北京 | 退服恢复情况、网络影响、基站退服、**时钟** |
| `company` | 分公司 | 同上 |
| `district` | 区 | 同上 |
| `street` | 街道 | 同上 |
| `community` | 社区 | 退服小区恢复情况、网络影响、基站退服、**时钟** |
| `station` | 物理站 | 退服小区恢复情况、指标、逻辑站清单、联保障任务、告警明细、**时钟** |
| `logical` | 逻辑站 | 退服小区恢复情况、站址画像、小区清单、站址性能、**时钟** |
...
> 日期：2026-08-26（更新：map-markers.json 引用变更（task008 计划 117 点位，L1 city 36 已完成）；history-timeline.json 与 6 类资源同构说明补充）
> 历史：
