...
tab2（突发保障 GIS）已切到 `<GisCustomCircleView>`（详见 [tab-content-2-aggregate-points.md § 5.6](tab-content-2-aggregate-points.md)），但 `onViewSetChange` / `onShowCircle` 仍用旧模式（`setCirclePoints([])` + `setTimeout`），未引入 `flushSync`。当前 `setCirclePoints` 写入前需 `pointArr.map(p => ({ ...p, neType: p.rawNeType ?? p.neType }))` 复原 neType（tab1 不需要这步），详见 [tab-content-2-aggregate-points.md § 5.3 `onShowCircle` 复原 neType](tab-content-2-aggregate-points.md)。
...
> tab2（突发保障 GIS）同样使用 `GisCustomCircleView`，v1.10 起也**传 `enableSelfPopup={true}`**（写法比 tab1 显式），详见 [tab-content-2-gis-full.md § `<GisCustomCircleView>`](tab-content-2-gis-full.md)。
...
| 自定位模式（div 手动定位）| ❌ 不支持                               | ✅ `enableSelfPopup`：用 `getPixelFromCoordinate` + `postrender` 事件手动定位                       |
...
| 顶层空数据守卫           | —                                       | ✅ `if (!visible \|\| !source?.length) return null;`（[CircleView.tsx#L102-L104](web/components/ui/oss-gis/CircleView.tsx#L102-L104)） |

### 几何 props + `enableSelfPopup`

```ts
interface CircleProps {
    enableSelfPopup?: boolean;     // ← v1.10 新增；true = 自定位模式
    source?: any[];
    onMouseMove?: any;
    onClick?: any;
    visible?: boolean;
    toolPupWindowId?: string;
...
### `enableSelfPopup` 自定位模式

传 `enableSelfPopup` 时，组件额外挂一个 `useEffect`（[CircleView.tsx#L74-L102](web/components/ui/oss-gis/CircleView.tsx#L74-L102)）：
```ts
useEffect(() => {
    if (!visible || !source?.length || !props.enableSelfPopup) {
        return; // 不再写 display；visibility 由内联 style + 顶层 return null 控制
    }
...
- **早返回不动 display**（v1.10 修复了之前 `dom.style.display = 'none'` 直接 DOM 写入覆盖 React 内联 style 的副作用）。`display` 由内联 `style={{ display: visible === true && !isEmpty(source) ? 'block' : 'none' }}` 控制
- `getPixelFromCoordinate([lng, lat])` 拿第一点的 OL 像素，再扣掉 `circleSize / 2`，使**容器中心**对齐第一点的像素位置
- `mapContext.on('postrender', updatePosition)` 订阅地图重绘事件，缩放/平移时自动跟随
- 卸载 / 重渲时用 `mapContext.un('postrender', updatePosition)` 清理
- 容器 id 变 `freePointContainer`，便于和默认 `pointContainer` 区分
...
- `toolPupWindowId` 仍然区分 tab：`toolTipWindowCircle1`（tab1）、`toolTipWindowCircle2`（tab2）
- 两个 Tab 都启用 `enableSelfPopup`（tab1 shorthand，tab2 `enableSelfPopup={true}` 显式写法）；切到自定位模式后容器 id 都是 `freePointContainer`，**不要在 CSS / 测试代码里硬编码 `pointContainer`**
- `enableSelfPopup` 只在 `visible=true && source 非空` 时实际定位；空数据时组件直接 `return null`（[CircleView.tsx#L102-L104](web/components/ui/oss-gis/CircleView.tsx#L102-L104)），不会在 DOM 里残留一个空容器

> 版本：v1.3 · 更新日期：2026-07-24（同步 v1.11：`toolTipWindowCircle1` 锚点 div 注释掉，聚合圆 tooltip 改走 `tooltipTileChildren`；调用方 `toolPupWindowId` 保留作 id 后缀约定）
